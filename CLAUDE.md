...
- [ ] `recent-pages.js` の `RECENT_PAGES` 配列の **先頭** に新規エントリを追加した（下記「新着コンテンツ管理」を参照）
...
## 新着コンテンツ管理（`recent-pages.js`）

`index.html` の「新着コンテンツ」セクションは、`recent-pages.js` の `RECENT_PAGES` 配列を `date` 降順でソートして上位6件を自動表示する。

**キャッシュバスティングは自動**：`index.html` は `recent-pages.js?v=YYYYMMDD` を `document.write` で動的生成し、日付ベースで cache key が毎日変わる。`?v=` を手動で bump する必要はない（過去 2 回手動 bump 忘れの事故あり）。

### 新規ページを追加するときの手順
1. ページを作成・公開した時点で **必ず** `recent-pages.js` を更新する。
2. 配列の **先頭**（最も新しい位置）に以下の形式で1エントリを追加する：

```js
{
  title: "ページの表示名（短めに）",                  // カードに表示されるタイトル
  url:   "pages/articles-gl-xxxx.html",              // 相対パス（index.html から見て）
  date:  "YYYY-MM-DD",                                // 公開日。ソートに使われるためISO形式厳守
  tag:   "論文" | "ガイドライン" | "解説" | "症例",    // 4種から1つ選ぶ
  thumb: "images/xxx.jpg"                            // サムネイル画像パス
}
```

### タグの使い分け
- **論文**：単一論文・原著（RCT・メタ解析等）
- **ガイドライン**：学会GL・公式レポート・専門医テキスト
- **解説**：レビュー・総説・教育的記事・疾患マニュアル
- **症例**：症例報告

### サムネイル選択
- 専用画像があれば優先（例：`images/IDICUtop.png` は ID×ICU関連）
- なければ汎用画像を使い回す（`icu-study.jpg` / `朝カンファ風景.webp` / `IDICUtop.png` 等）
- 同時期に追加する複数ページでサムネイルが連続しないよう、配列内で交互に並べる

### 注意事項
- **配列が空・JSが読み込み失敗の場合は新着コンテンツは表示されない**（フォールバック無し）。必ず動作確認する。
- 古いエントリは削除しなくてよい（上位3件のみ表示されるため）。長期的にはトリミングしてもよい。
- `date` が同日の場合、配列内の並び順が維持される（先頭のものが上に表示）。
