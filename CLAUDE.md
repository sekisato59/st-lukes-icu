...
**🔴 最優先ルール：赤いテキストボックス（`alert-red`・`danger` 等の赤箱）の中では、マーカーは `mark-red` のみを使う。**
赤箱内に黄/緑/橙マーカーがあると非常に見栄えが悪く、意味も衝突する（赤箱＝危険/警告の文脈なので「推奨(緑)」「注意(黄)」等は矛盾）。
赤箱内の強調はすべて `mark-red` に統一する。この規則は下記の「意味で選ぶ」より**優先**される。
監査・一括修正は `node scripts/audit-red-markers.js`（`--fix` で赤箱内の非赤マーカーを mark-red に統一）。
