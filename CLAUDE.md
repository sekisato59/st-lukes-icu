...
**図（Figure）の取り込み＝既定で搭載・切り出しは satoruss ツールに統一（2026-06-25）：**
PDF の図（代表症例・フローチャート・フォレストプロット・MRI・写真等）は**指示がなくても既定で取り込む**（省略しない）。再現可能なものは HTMLコンポーネント（`case-box`／`ie-flow`／`ie-table` 等）に起こし、再現不能な実図は**画像として挿入**する。**画像で切り出す時は satoruss の共通ツールを使う**（AIが座標を当てるより確実・効率的）：
```bash
# ① 広めのざっくり枠を渡す → autobox.py が図の実バウンディングボックスへ自動スナップ（枠線は残す）
node ~/.claude/skills/satoruss/extract-figure.mjs "<pdf>" <頁> --dpi 300 --crop X,Y,W,H --out fig.png
#    雑誌キャプション（図の下のテキスト塊）を外す時は --drop-caption
# ② 全図を1つの自己完結HTMLにまとめて open し、ユーザー確認を得てから本文に入れる
node ~/.claude/skills/satoruss/figures-review.mjs /tmp/figs.html "fig.png|Figure 1 ○○の推移"
```
切り出した図は本文と同じ白い箱に入れる（[[feedback_figure_related_in_box]]）。Claude Code 内で1枚ずつ見せず、必ず HTML に集約して確認する。
...
- **🚫【絶対禁忌・二重丸】丸囲み数字グリフ（①②③…㉑…）を、背景色付き／円形のバッジの中に入れない。** 円形バッジ（`border-radius:50%`）や色付きピル（`ie-score-pt` 等）の中に `①` を置くと、バッジの円とグリフの円が二重に見えて非常に不快。**バッジ内の番号は必ずプレーン数字（`1` `2` `3`）にする。** 緑（テーマ色）の丸・ピルは残してよいが、中身は丸囲みグリフではなくただの算用数字。該当コンポーネント：`sero-num`／`bundle-num`／`ie-flow-num`／`ie-score-pt`／`step-num` ほか色付きバッジ全般。例外：`summary-num` のような「背景なし・幅18pxのプレーン色文字」や、見出し・本文・リスト内の番号プレフィックスとしての `①`（バッジでない）は許可。監査：`grep -rnoE '<(span|div)[^>]*(border-radius: ?50%|ie-score-pt|bundle-num|sero-num)[^>]*>[ ]*[①-⑳]' pages` が**0件**であること。
...
- `mark-green` / `mark-blue` / `mark-red` — 下線ハイライト（`mark-yellow` / `mark-orange` / `mark-pink` は原則使わない）
- 使い方: `<span class="mark-green"><strong>重要テキスト</strong></span>`

**⚠️ マーカー色は「ページのテーマカラーの寒暖」で2色に絞る（2026-06-17 改訂・必須遵守）。**

旧来の「意味で選ぶ（緑=推奨／赤=禁忌／黄=注意／橙=補足）」スキームは**廃止**。理由：寒色テーマの青いページに赤・橙・黄マーカーが散乱すると非常に見栄えが悪い。**テーマと調和する2色だけ**を使う。

| ページのテーマ | 使うマーカー2色 |
|---|---|
| **寒色系**（青・緑・藍・インディゴ・紫など。例：神経=青、ID×ICU=紫、試験ノート=インディゴ） | **緑（`mark-green`）＋青（`mark-blue`）** |
| **暖色系**（オレンジ・赤・茶など。例：疾患トピックス=オレンジ） | **緑（`mark-green`）＋赤（`mark-red`）** |

- **緑は両テーマ共通**。第2色だけがテーマで変わる（寒色→青／暖色→赤）。
- 1ページ内のマーカーは**この2色まで**。3色以上に散らさない・色をローテーションさせて「賑やかし」にしない。
- `mark-blue` は style-v2.css に定義済み（寒色ページ用）。共通CSSが未反映の懸念があるページでは、ページ内 `<style>` に同形式でローカル定義してよい。
- 厳密な「禁忌＝赤」等の意味付けはしない。強調したい語に、テーマ調和色（緑か第2色）を付けるだけでよい。

**🔴 赤箱ルール（暖色ページのみ）：赤いテキストボックス（`alert-red`・`danger`）の中ではマーカーは `mark-red` のみ。**
赤箱は暖色ページにのみ置く（寒色ページに赤箱・赤マーカーは原則置かない）。監査・一括修正は `node scripts/audit-red-markers.js`（`--fix` で赤箱内の非赤マーカーを mark-red に統一）。

**🟡 黄箱ルール（ページのテーマに関係なく適用）：黄色/琥珀のテキストボックス（`alert-yellow`・`notice-box`・背景が `#FFFBEB`/`#FEF3C7`/`#FFF7ED` 等の暖色淡色）の中では、寒色マーカー（`mark-blue`/`mark-green`/`mark-pink`）と地に埋もれる `mark-yellow`・紛らわしい `mark-orange` を使わない。黄箱内の強調は暖色アンバー下線マーカー `mark-amber` に統一する（`mark-red` のみ意図的な警告として残してよい）。**
理由：淡いクリーム/琥珀地に青・緑の寒色マーカーを乗せると寒色×暖色で非常に見栄えが悪く、黄マーカーは地に埋もれて読めない。`mark-amber`（`style-v2.css` 定義済み・`rgba(245,158,11,0.32)` の下線）は地と同系の暖色で馴染みつつ十分に視認できる。**寒色テーマのページでも、黄箱の“中だけ”はこのルールが優先する**（ページ全体のマーカー2色ルールより箱が優先）。監査・一括修正は `node scripts/audit-yellow-markers.js`（`--fix` で黄箱内の非アンバー＝red以外のマーカーを mark-amber に統一）。
...
- セクション左ボーダー: **常に `#2D7A4F`（テーマ緑で統一）**。旧来の赤橙緑ローテーションは廃止（2026-06-22）。`.section-label`（SECTION ラベル）は常に緑のため、ボーダーを赤/橙にすると「緑文字＋橙の帯」のちぐはぐな配色になる。寒色テーマに赤橙を散らさない原則（2026-06-17 マーカー規律）と整合させ、ボーダーは緑単色にする
...
- セクション左ボーダー: **常に `#B45309`（テーマ橙で統一）**。旧来のローテーションは廃止（2026-06-22）。SECTION ラベルとボーダーは必ず同色（テーマ色）にする
