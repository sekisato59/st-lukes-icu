...
// ===== AI質問ボタン（NotebookLM） =====
(function initAIFab() {
  var NLM_URL = 'https://notebooklm.google.com/notebook/ad1d0265-a784-465c-b620-6e6458de9316';

  function openNotebookLM() {
    var sw = window.screen.width;
    var sh = window.screen.height;
    var w = Math.min(920, sw - 40);
    var h = Math.min(720, sh - 60);
    var left = sw - w - 10;
    ...
    if (!popup) window.open(NLM_URL, '_blank');
  }

  function createFab() {
    var btn = document.createElement('button');
    btn.className = 'ai-fab';
    btn.setAttribute('aria-label', 'AIに質問する（NotebookLM）');
    btn.setAttribute('title', 'AIに質問する（NotebookLM）');
    btn.innerHTML =
      '<svg viewBox="0 0 24 24" aria-hidden="true">' +
        '<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>' +
      '</svg>AIに質問';
    btn.addEventListener('click', openNotebookLM);
