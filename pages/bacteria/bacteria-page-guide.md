...
| `id-icu-notes/note-*.html` | **link**（原則） | ページ固有クラスをローカル `<style>` で定義しており、埋め込むと無装飾で崩れる |
| note の **drug-block 系**（標的治療ボックス） | **embed 可** | `drug-block` の CSS を bp.css の `.bp-rel-embed` 配下に移植済み（例：MSSA/MRSA 標的治療） |
